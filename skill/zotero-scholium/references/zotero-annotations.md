...
Body column edges are the 2nd and 98th percentiles of word x-coordinates (header and footer excluded). Two-column pages are detected by a small number of words crossing the page centre. A summary box is placed beside the first line of its anchor phrase, on the paragraph's side (two-column layouts) or in the wider margin; boxes on one side are shifted downward to avoid overlap and moved upward if they would extend into the footer. Existing text, note, image and ink annotations that are not the tool's own are read from Zotero before layout and treated as occupied space; boxes that cannot be placed without overlap are reported in `layout_warnings`. Previews are drawn on in-memory pages; the document is never saved.
